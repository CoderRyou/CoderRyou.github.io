<template>
  <main class="page-container">
    <div class="page-content">
      ...
      <SeriesRecommend v-if="isShowSeries" />
...
const { pageLoaded, pagePasswordPass, setedPagePassword, handlePass } =
  usePassword()

const title = computed(
  () => page?.value?.frontmatter?.title || page?.value?.title || ''
)

const sticky = computed(() => page?.value?.frontmatter?.sticky || '')

const isShowSeries = computed(() => {
  const { series } = page?.value?.frontmatter
  return !!series
})
